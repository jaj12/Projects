...
W=L/2;%range for y-coordinates 

Xp=51;%number of x surfaces
Yp=31;%number of y surfaces
...
%% input parameters
Qg=50; %heat generation W/m3

Tc=345*ones(n-1,m-1); %temperature matrix in Kelvin Initial Guess
Tcold=Tc;
...
for k=1:ItMAX
            ...
            [TbN,TbS,TbE,TbW]=borderTemp(Tc,TbN,TbS,TbE,TbW,BW,BE,BN,BS,QbW,QbE,QbS,QbN,hbW,hbE,hbS,hbN,TinfW,TinfE,TinfS,TinfN,dx,dy,dxc,dyc);
            
            %obtaining coefficients for the energy balance at each CV.
            [a,an,as,aw,ae,b]=CoeffCal(dxc,dyc,dy,dx,Qg,BE,BW,BN,BS,hbE,hbW,hbN,hbS,TinfE,TinfW,TinfN,TinfS,TbE,TbW,TbN,TbS,QbE,QbW,QbN,QbS,kn,ks,ke,kw);
...
figure(1)
surf(xc,yc,Tc')
%TbN TbS TbE TbW are the boundary temperatures
xlabel('South'), ylabel('West'), zlabel('Temperature (K)')
