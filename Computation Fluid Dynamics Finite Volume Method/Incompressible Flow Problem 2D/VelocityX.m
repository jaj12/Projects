function [aw,an,ae,as,a,b,dpcx,dpcy,Pwb,Peb,Pnb,Psb]=VelocityX(P,dxc,dyc,dy,dx,BE,BW,BN,BS,WN,WE,WS,WW,Ux,Uy,ConvectionScheme,Rho,dt,TransientScheme,UNiny,USiny,UNinx,USinx,UEinx,UWinx,Uwx,Uex,Uny,Usy,Uxoldtrans,TbN,TbS,TbE,TbW,n2i,s2i,e2i,w2i,PbW,PbE,PbS,PbN)
    ...
    for i=1:n-1
        for j=1:m-1
            ...
            if i==1 %West
                ...
                dUxw(i,j)=(Ux(i,j)-UbWx(j))/dxc(i);
                if WWi==9%no slip
                aby=-mbuw(j)*(dy(j)/(dxc(i)));
                Pwb(i)=P(i,j)-dpcx(i,j)*dxc(i);
                Pwt=Pwb(j);
                bbwx=Pwb(i)*dy(j)-aby*UWwallx;
                ...
                bbwx=-mdotw*UWinx+mbuw(j)*(dy(j)/(dxc(i)))*UWinx+Pwb(j)*dy(j);
                awt=awt+mdotw;
                ...
                dUxw(i,j)=(Ux(i,j)-Ux(i-1,j))/dxc(i);
            ...
            if i==n-1 %East
                ...
                dUxe(i,j)=-(Ux(i,j)-UbEx(j))/dxc(i+1);
                if WEi==9
                aby=-mbue(j)*(dy(j)/(dxc(i+1)));
                Peb(j)=P(i,j)+dpcx(i,j)*dxc(i+1);
                Pet=Peb(j);
                bbex=Peb(j)*dy(j)-aby*UEwallx;
                ...
                bbex=-mdote*UEinx+mbue(j)*(dy(j)/(dxc(i+1)))*UEinx+Peb(j)*dy(j);
                aet=aet+mdote;
                elseif BEi==5
                ...
                dpcx(i,j)=(P(i+1,j)-P(i-1,j))/(dxc(i)+dxc(i+1));
                dpcy(i,j)=(P(i,j+1)-P(i,j-1))/(dyc(j)+dyc(j+1));
                dUxe(i,j)=(Uy(i+1,j)-Uy(i,j))/dxc(i+1);
            ...
            if j==1 % South
                ...
                dUxs(i,j)=(Ux(i,j)-UbSx(i))/dyc(j);
                if WSi==9
                abx=-mbus(j)*(dx(i)/(dyc(j)));
                Psb(i)=P(i,j)-dpcy(i,j)*dyc(j);
                Pst=Psb(i);
                bbsy=Psb(j)*dx(i)-abx*USwallx;
                ...
                bbsy=-mdots*USinx+mbus(i)*(dx(i)/(dyc(j)))*USinx+Psb(i)*dx(i);
                ast=ast+mdots;
                elseif BSi==5
                elseif BSi==6
                elseif BSi==7
                ...
                dpcx(i,j)=(P(i+1,j)-P(i-1,j))/(dxc(i)+dxc(i+1));
                dpcy(i,j)=(P(i,j+1)-P(i,j-1))/(dyc(j)+dyc(j+1));
                dUxs(i,j)=(Ux(i,j)-Ux(i,j-1))/dyc(j);
            ...
            if j==m-1 % North
                    ...
                    dUxn(i,j)=-(Ux(i,j)-UbNx(i))/dyc(j+1);
                    if WNi==1
                    abx=-mbun(i)*(dx(i)/(dyc(j+1)));
                    Pnb(i)=P(i,j)+dpcy(i,j)*dyc(j+1);
                    Pnt=Pnb(i);
                    bbny=Pnb(i)*dx(i)-abx*UNwallx;
                    ant=abx;
                    elseif WNi==2
                    Pnb(i)=P(i,j)+dpcy(i,j)*dyc(j+1);
                    bbny=Pnb(i)*dx(i);
                    ...
                    bbny=-mdotn*UNinx+mbun(j)*(dx(i)/(dyc(j+1)))*UNinx+Pnb(i)*dx(i);
                    ant=ant+mdotn;
                    elseif BNi==5
                    elseif BNi==6
                    elseif BNi==7
                   ...
                   dpcx(i,j)=(P(i+1,j)-P(i-1,j))/(dxc(i)+dxc(i+1));
                   dpcy(i,j)=(P(i,j+1)-P(i,j-1))/(dyc(j)+dyc(j+1));
                   dUxn(i,j)=(Uy(i,j+1)-Uy(i,j))/dyc(j+1); 
          ...
          %%  Convection Coefficients
            ...
            if j~=m-1
                ...
                if i==1 %West
                    ...
                    Uxfdimw=Smart(Uxdimw);
                    Uxfw=Uxfdimw*Qw+Uxuw;
                    bdcw=mdotw*(Uxfw-Uxcw);
                end
                ...
                if j==1 %South
                    ...
                    Qs=Uxds-Uxus;
                    if Qs~=0
                        Uxcdims=(Uxcs-Uxus)/Qs;
                    else
                        Uxcdims=0;
                    end
                    Uxfdims=Smart(Uxcdims);
                    Uxfs=Uxfdims*Qs+Uxus;
                    bdcs=mdots*(Uxfs-Uxcs);
                end
                ...
                if i==n-1 %East
                    ...
                    Qe=Uxde-Uxue;
                    if Qe~=0
                        Uxcdime=(Uxce-Uxue)/Qe;
                    else
                        Uxcdime=0;
                    end
                    Uxfdime=Smart(Uxcdime);
                    Uxfe=Uxfdime*Qe+Uxue;
                    bdce=mdote*(Uxfe-Uxce);
                end
                ...
                if j==m-1 %North
                    ...
                    Qn=Uxdn-Uxun;
                    if Qn~=0
                        Uxcdimn=(Uxcn-Uxun)/Qn;
                    else
                        Uxcdimn=0;
                    end
                    Uxfdimn=Smart(Uxcdimn);
                    Uxfn=Uxfdimn*Qn+Uxun;
                    bdcn=mdotn*(Uxfn-Uxcn);
                end
            ...
            bttot=dUxw*dy(j)+dUxe*dy(j)+dUxn*dx(i)+dUxs*dx(i); %transpose diffusion term
            ...
            b(i,j)=bbvisc-bdctot+bbctot+bbtot+bptot+bttot+a0*Uxoldtrans(i,j)+Fb*dx(i)*dy(j);
        end
    end
end